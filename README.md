...
Introducing `defaultUsers` and `pricePerUser` to the plans schema will impact the current plan upgrade calculation by necessitating the inclusion of user count in the calculation of the daily rate. Previously, the daily rate was calculated based solely on the price difference between plans. With these new properties, the calculation must now also account for the number of users included by default and the additional cost per user beyond the default allocation. This ensures that the prorated upgrade price accurately reflects the total cost based on the number of users and the remaining days in the current billing cycle.
